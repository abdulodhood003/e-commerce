...
const PlaceOrder = () => {
  ...
  const initPay = (order) => {
    const options = {
      ...
      handler: async (response) => {
        try {
          const { data } = await axios.post(`${backendUrl}/api/order/verifyRazorpay`, { ...response, userId: token?.id }, { headers: { token: token } });
          if (data.success) {
            navigate('/orders');
            setCartItems({});
          } else {
  ...
  const onSubmitHandler = async (event) => {
    ...
    try {
      ...
      for (const items in cartItems) {
        for (const item in cartItems[items]) {
          if (cartItems[items][item] > 0) {
            const itemInfo = JSON.parse(JSON.stringify(
      ...
      if (method === 'stripe') {
        const resStripe = await axios.post(`${backendUrl}/api/order/stripe`, orderData, { headers });
